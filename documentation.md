...
6. [JavaScript Interactivity (Week 3 Addition)](#6-javascript-interactivity-week-3-addition)
   - [6.1 Theme Toggle & Local Storage](#61-theme-toggle--local-storage)
   - [6.2 Show/Hide Content (Read More)](#62-showhide-content-read-more)
   - [6.3 Interactive Image Slider](#63-interactive-image-slider)
   - [6.4 Form Validation](#64-form-validation)
7. [Technical Requirements Checklist](#7-technical-requirements-checklist)
8. [Visual Documentation (Screenshots)](#8-visual-documentation-screenshots)
9. [Testing Evidence](#9-testing-evidence)
10. [Concepts Learned & Reflections](#10-concepts-learned--reflections)
...
## 6. JavaScript Interactivity (Week 3 Addition)

This project has been upgraded with **JavaScript** to introduce client-side interactivity, DOM manipulation, and dynamic user experiences without relying on a backend server. All JS logic is in a dedicated `script.js` file linked before the closing `</body>` tag.

### 6.1 Theme Toggle & Local Storage
I've implemented a Light/Dark Mode toggle button in the navigation bar. By default, the site is in dark mode. 
- **DOM Manipulation:** The script toggles a `.light-mode` class on the `<body>` element. This class overrides the CSS Custom Properties (variables) defined in `:root`, instantly swapping the color scheme.
- **Local Storage:** The theme preference is saved in the browser’s `localStorage`. When the page reloads or the user visits again, a script running on `DOMContentLoaded` retrieves the value (`light` or `dark`) and automatically applies it, providing a persistent user experience.

### 6.2 Show/Hide Content (Read More)
To improve the initial layout while keeping detailed information accessible, the **About Me** section features a "Read More" button.
- **Event Listeners:** A click listener changes the text of the button between "Read More" and "Read Less".
- **Dynamic CSS:** It toggles a `.show` class on a hidden `<span>` that switches its `display` property from `none` to `block`.

### 6.3 Interactive Image Slider
A new **Projects** section was created specifically to host an interactive slider.
- **Logic:** The script tracks the `currentSlide` index. Clicking "Next" or "Prev" updates this index using modulo arithmetic (`%`) to automatically loop back around when reaching the ends.
- **Animation:** The DOM is manipulated by updating the inline `transform: translateX(...)` style of the slider wrapper, coupled with a CSS `transition` for a smooth sliding animation.

### 6.4 Form Validation
Client-side form validation prevents the user from submitting empty or incorrectly formatted data and offers real-time feedback.
- **Event Handling:** Hooked into the form's `submit` event, using `event.preventDefault()` to stop the page from refreshing.
- **Validation Rules:**
  - **Name:** Ensures the input is at least 3 characters long.
  - **Email:** Uses Javascript Regex (`/^[^\s@]+@[^\s@]+\.[^\s@]+$/`) to ensure a valid email format.
  - **Message:** Ensures the textarea has at least 10 characters.
- **Error/Success Messaging:** Uses reusable functions (`showError`, `clearError`) to display hidden `.error-message` spans and outline inputs in red when validation fails. If all checks pass, it reveals a `.success-message` div and clears the form.
...
| Create `script.js` & Link | ✅ Done | Added at bottom of body |
| Form Validation | ✅ Done | Name, email regex, message minimum chars, and success feedback |
| 3+ Interactive Features | ✅ Done | Theme Toggle, Read More, Slider implemented |
| Local Storage | ✅ Done | Persists the user's light/dark mode preference |
...
The full-width layout showing the sticky glassmorphism header, About section with profile image, Skills grid, Projects slider, and Contact form.
...
## 10. Concepts Learned & Reflections
